...
# Destroying there home before they destroy us...
...
class Gaiaquery:  #quering the data from Gaia 
    """Queries the data from GAIA and makes it usable"""
...
class Star:
    """Defines all the necessary things for the stars queried"""
    ...
    def __repr__(self):  #gives the star id and distance
        return f"Star(id={self.source_id}, dist={self.distance:.2f} pc)"

    def get_spectral_type(self):  #defines the spectral type
        t = self.temperature
        if t is None:
            return None
        elif t > 30000:
...
def scramble(word):  #scrambles the letters and makes sure its not the right word
...
class PlanetAnagramPuzzle:
    ...
    def play(self):
        ...
        print("Agent: But, be careful, we have only one change... The invaders are close...")
